...
public:
    ...
    void setInitReferenceState(vector<float> refState_){
        _referenceState = refState_;
    }

    void computeNextReferenceState(){
        //identity_matrix<float> I(6);
        vector<float> State(6);
        //deltaState = prod((I + step_*A(_state)), _trueState - _state);
        //computeNextState();
        State = _referenceState + _deltaStateEstimateFK;
